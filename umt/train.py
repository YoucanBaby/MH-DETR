...
def train_one_epoch(model, criterion, train_loader, optimizer, opt, epoch_i, tb_writer):
...
def train(model, criterion, optimizer, lr_scheduler, train_dataset, val_dataset, opt):
    ...
    for epoch_i in trange(start_epoch, opt.epochs, desc="Epoch"):
        if epoch_i > -1:
            train_one_epoch(model, criterion, train_loader, optimizer, opt, epoch_i, tb_writer)
...
def start_training():
    ...
    if 1:
        ...
        dataset_config = dict(
            ...
            txt_drop_ratio=opt.text_drop_ratio
        )
